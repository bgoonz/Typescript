...
- Setting the above to true is equivalent to enabling the following features:
...
```bash
$ tsc
src/analytics/analytics.ts:2:25 - error TS7006: Parameter 'data' implicitly has an 'any' type.

2 function sendAnalytics (data){
                          ~~~~


Found 1 error in src/analytics/analytics.ts:2
```

- if we set the noImplicitAny option to false then the above error will not be generated.
